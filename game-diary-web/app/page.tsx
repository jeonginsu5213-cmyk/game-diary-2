    ...
    <div className="relative group/slider mt-2 mb-4 font-sans text-[#1A1D1F]">
      ...
      <div ref={scrollRef} className="flex gap-5 overflow-x-auto pb-4 scrollbar-hide snap-x snap-mandatory px-1 font-sans">
        {screenshots.map((shot: any, idx: number) => {
          ...
          return (
            <div key={idx} className="relative shrink-0 w-72 snap-start flex flex-col group/unit font-sans">
  ...
  return (
    <div className="mt-2 flex items-center gap-3 p-2 bg-white rounded-2xl border border-slate-100 shadow-inner group/input font-sans">
      ...
      <input 
        ...
        onKeyDown={onKeyDown}
        placeholder="이 게임에 대한 한줄평을 남겨주세요..." 
 ...
 * 메인 일기 카드 컴포넌트
 */
const DiaryCard = ({ data, playersSet, sessionId }: any) => {
  const [isParticipantsExpanded, setIsParticipantsExpanded] = useState(false);
  // 🌟 필터링 로직 추가: gameTitle이 없거나 "미지정"인 것만 분류되지 않은 섹션에 표시
  ...
  const moveImageToGame = async (imgUrl: string, gameTitle: string) => {
    ...
    try {
      const sessionRef = doc(db, "sessions", sessionId);
      // 스크린샷 데이터의 gameTitle 속성만 업데이트하여 제자리로 찾아가게 함
  ...
  return (
    <article className="bg-white/70 backdrop-blur-md border border-white p-8 md:p-12 rounded-[3rem] shadow-[0_20px_50px_rgb(0,0,0,0.05)] mb-10 font-sans text-[#1A1D1F]">
      <div className="flex flex-col gap-6 mb-10">
        <div className="flex items-center justify-between font-sans">
          <div className="flex items-center gap-4">
            <span className="font-black bg-[#1A1D1F] text-white px-3 py-1 rounded-full uppercase tracking-tighter shrink-0 text-xs font-sans">{data.date}</span>
            <h2 className="font-black tracking-tight text-2xl font-sans">{data.sessionTitle}</h2>
          </div>
          
          <div className="flex items-center gap-2 shrink-0">
            <button className="p-2.5 bg-[#F0F2F5] hover:bg-[#E2E8F0] rounded-full transition-all border border-slate-200 cursor-pointer" title="제목 수정">
        ...
        <div className="flex flex-col gap-4 bg-slate-50/50 p-5 rounded-[2rem] border border-slate-100/50 transition-all hover:bg-white hover:shadow-md">
          <div className="flex items-center gap-8">
            <div className="flex flex-col border-r border-slate-200 pr-8 shrink-0">
              <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1.5 font-sans">총 대화 시간</span>
              <div className="flex items-baseline gap-1 text-[#1A1D1F]"><span className="text-xl font-black font-sans">{data.totalDurationMin}</span><span className="text-[10px] font-bold text-gray-400 font-sans">분</span></div>
            </div>
          ...
          {isParticipantsExpanded && (
            <div className="pt-4 mt-2 border-t border-slate-100 grid grid-cols-2 sm:grid-cols-4 gap-3 font-sans">
      ...
      <div className="grid grid-cols-1 gap-8 mb-16 font-sans">
        ...
        {data.games.map((game: any, i: number) => {
          // 🌟 로직 수정: 전체 스크린샷 중 해당 게임 타이틀과 일치하는 것만 필터링
          const gameShots = (data.unclassifiedScreenshots || []).filter(
            (s: any) => s.gameTitle === game.title
          );

          return (
            <div key={i} className="group relative bg-white border border-slate-100 shadow-sm p-6 rounded-[2rem] font-sans">
              <div className="flex flex-col md:flex-row justify-between gap-6 mb-8 font-sans">
                <div className="flex gap-4 text-[#1A1D1F] font-sans">
                  <div className="flex flex-col items-center shrink-0 justify-center font-sans"><div className="w-16 h-16 rounded-xl bg-[#F8F9FA] flex items-center justify-center border border-slate-100 shadow-inner overflow-hidden font-sans">{game.iconURL ? <img src={game.iconURL} alt="" className="w-full h-full object-contain" /> : <span className="text-2xl font-sans">🕹️</span>}</div></div>
                  <div className="font-sans">
                    <h4 className="font-black tracking-tight mb-3 uppercase text-lg font-sans">{game.title}</h4>
                    <div className="flex flex-wrap gap-x-3 gap-y-2 font-sans">
                      {game.players?.map((player: string) => (
                        <div key={player} className="flex items-center gap-2 bg-[#1E1F22] px-2 py-1 rounded-full shadow-sm text-[#DBDEE1] font-sans">
                          <div className="w-5 h-5 rounded-full overflow-hidden border border-white/10 font-sans">
                          ...
                          <span className="text-[10px] font-bold font-sans">{data.displayNames[player] || player}</span>
                          <span className="text-[10px] font-black text-[#949BA4] ml-1 font-sans">{game.playTimeMin}분</span>
                        </div>
                      ))}
                    </div>
                  </div>
                </div>
                <div className="md:text-right shrink-0 flex flex-col justify-center font-sans text-[#1A1D1F]"><div className="text-[9px] font-black text-gray-400 uppercase mb-0.5 font-sans">총 플레이 시간</div><div className="font-black text-[#1A1D1F] text-2xl font-sans">{game.playTimeMin}분</div></div>
              </div>
              ...
              <div className="mt-2 pt-4 border-t border-slate-50 font-sans text-[#1A1D1F]">
                ...
                <div className="space-y-2 font-sans">
                  ...
                  <GameCommentInput 
                    ...
                    currentUser={data.participants[0]} 
                    data={data}
                  />
                </div>
              </div>
      ...
      <div className="space-y-6 font-sans text-[#1A1D1F]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 font-sans">
          {localUnclassified.map((snap: any) => {
            ...
            return (
              <div key={snap.url} className="bg-white p-4 rounded-[2.5rem] border border-slate-50 shadow-sm relative group/snap-unit font-sans transition-all hover:shadow-md">
                <div className="relative aspect-video rounded-[1.8rem] overflow-hidden border border-slate-100 shadow-inner group/snap font-sans">
                  ...
                  <div className="absolute bottom-3 left-3 opacity-0 group-hover/snap:opacity-100 transition-all font-sans">
                    <select 
                      onChange={(e) => moveImageToGame(snap.url, e.target.value)} 
                      className="bg-[#1E1F22]/90 text-white text-[9px] font-black px-3 py-1.5 rounded-full border border-white/10 shadow-xl outline-none cursor-pointer hover:bg-black font-sans"
                      defaultValue=""
                    >
                  ...
                  <DownloadButton url={snap.url} className="absolute top-3 right-3 opacity-0 group-hover/snap:opacity-100 transition-all font-sans" />
                </div>
                {snap.comment && <PhotoCaption text={snap.comment} />}
              </div>
            );
          })}
          <div className="group/upload cursor-pointer h-full min-h-[220px] bg-slate-50/50 border-2 border-dashed border-slate-200 rounded-[2.5rem] flex flex-col items-center justify-center gap-4 transition-all hover:bg-white hover:border-slate-300 font-sans">
            <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-slate-400 font-sans"><svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M17 8l-5-5-5 5M12 3v12" /></svg></div>
            <p className="text-xs font-black text-slate-400 font-sans">스크린샷 업로드</p>
          </div>
        </div>
      </div>
    </article>
...
export default function Home() {
  ...
  if (sessions.length === 0) return (
    <div className="min-h-screen flex items-center justify-center bg-[#F0F2F5] text-[#1A1D1F] font-sans">
      <div className="bg-white p-12 rounded-[3rem] shadow-sm text-center border border-slate-100 font-sans">
  ...
  return (
    <div className="flex min-h-screen bg-[#F0F2F5] text-[#1A1D1F] font-sans overflow-hidden font-sans">
      <nav className="w-16 bg-[#1A1D1F] flex flex-col items-center py-6 gap-8 shrink-0 font-sans">
        <div className="w-8 h-8 bg-white/10 rounded-xl flex items-center justify-center text-white font-black text-xl font-sans">G</div>
        <div className="mt-auto w-10 h-10 rounded-full border-2 border-white/20 overflow-hidden font-sans shadow-lg">
          <img src={`https://api.dicebear.com/7.x/adventurer/svg?seed=yongmyeong&backgroundColor=b6e3f4`} alt="" className="w-full h-full" />
        </div>
      </nav>

      <aside className="w-72 bg-[#F8F9FA] border-r border-[#E2E8F0] p-6 flex flex-col gap-6 font-sans">
        <div className="space-y-3 font-sans">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-wider px-1 font-sans">참여중인 서버</p>
          <div className="flex items-center gap-3 p-2 bg-white rounded-2xl border border-slate-100 shadow-sm font-sans transition-all hover:shadow-md">
            ...
            <div className="flex flex-col overflow-hidden font-sans">
              ...
              <span className="text-[10px] font-medium text-green-500 flex items-center gap-1.5 font-sans"><span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse font-sans" />연동됨</span>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-2 overflow-y-auto pr-2 scrollbar-hide font-sans">
            {sessions.map(s => (
              <button 
                ...
                className={`w-full text-left p-3 rounded-2xl border transition-all group font-sans ${selectedId === s.id ? 'bg-white border-[#1A1D1F]/10 shadow-md translate-x-1' : 'bg-transparent border-transparent hover:bg-white/50 grayscale hover:grayscale-0'}`}
              >
                <div className="flex items-start gap-3 font-sans">
                  ...
                  <div className="flex flex-col min-w-0 font-sans">
                    <span className="text-[9px] font-black text-gray-400 uppercase mb-1 font-sans">{s.date}</span>
      ...
      <section className="flex-1 overflow-y-auto p-6 md:p-8 font-sans text-[#1A1D1F]">
        <header className="flex justify-between items-center mb-6 font-sans">
          <div className="flex items-center gap-4 p-2 pr-6 bg-[#1E1F22] rounded-[1.5rem] shadow-xl border border-white/10 cursor-default font-sans">
            <div className="w-12 h-12 rounded-2xl bg-[#313338] flex items-center justify-center border border-white/5 overflow-hidden font-sans shrink-0">
              <img src={current?.serverIconURL} alt="" className="w-9 h-9" />
            </div>
            <div className="flex flex-col font-sans">
              <span className="text-[14px] font-black text-[#DBDEE1] uppercase tracking-tight font-sans">{current?.serverName}</span>
              <div className="flex items-center gap-2 mt-1.5 font-sans"><span className="w-2 h-2 rounded-full bg-green-500 animate-pulse font-sans" /><span className="text-[10px] font-bold text-[#949BA4] uppercase tracking-widest font-sans">실시간 기록 연결중</span></div>
            </div>
          </div>
        </header>
        
        {current && <DiaryCard key={current.id} data={current} playersSet={playedUsers} sessionId={current.id} />}
      </section>
    </div>
  );
}