...
export async function DELETE(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    replyId = typeof body.replyId === "string" ? body.replyId : "";
  } catch {
    return withMobileCors(NextResponse.json({ error: "Invalid request body" }, { status: 400 }), request);
  }
  if (!replyId || replyId.length > 200) {
    return withMobileCors(NextResponse.json({ error: "Invalid reply id" }, { status: 400 }), request);
  }
  ...
  const { data: game, error: gameError } = gameResult;
  if (participantError || gameError) {
    console.error("Failed to verify mobile reply deletion:", participantError?.message || gameError?.message);
    return withMobileCors(NextResponse.json({ error: "Failed to delete reply" }, { status: 500 }), request);
  }
  if (!participant || participant.is_deleted === true) {
    return withMobileCors(NextResponse.json({ error: "Session not found" }, { status: 404 }), request);
  }
  ...
  const { data: parentComment, error: parentCommentError } = await supabase
    ...
    .maybeSingle();
  if (parentCommentError) {
    console.error("Failed to load mobile reply parent for deletion:", parentCommentError.message);
    return withMobileCors(NextResponse.json({ error: "Failed to delete reply" }, { status: 500 }), request);
  }
  if (!parentComment) {
    return withMobileCors(NextResponse.json({ error: "Parent comment not found" }, { status: 404 }), request);
  ...
  const replies = Array.isArray(parentComment.replies) ? parentComment.replies : [];
  const reply = replies.find((item) => item && typeof item === "object" && item.id === replyId && item.userId === mobileSession.userId);
  if (!reply) {
    return withMobileCors(NextResponse.json({ error: "Reply not found" }, { status: 404 }), request);
  }

  const { error: updateError } = await supabase
    .from("comments")
    .update({ replies: replies.filter((item) => item?.id !== replyId) })
    .eq("id", parentComment.id)
    .eq("game_id", game.id);
  if (updateError) {
    console.error("Failed to delete mobile reply:", updateError.message);
    return withMobileCors(NextResponse.json({ error: "Failed to delete reply" }, { status: 500 }), request);
  }

  return withMobileCors(NextResponse.json({ deletedReplyId: replyId }), request);
}
