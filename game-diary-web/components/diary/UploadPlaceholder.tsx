...
const UploadPlaceholder = ({ onFileSelect, className }: { onFileSelect: (file: File) => void; className?: string }) => {
  ...
  return (
    <div 
      ...
      onDrop={onDrop}
      className={`relative flex flex-col items-center justify-center gap-2 cursor-pointer transition-all duration-300 group/upload ${
        active 
          ? 'bg-primary/10 shadow-sm' 
          : 'bg-black/5'
      } ${className ?? 'rounded-[6px] aspect-video'}`}
    >
      <input 
        type="file" 
        ref={fileInputRef} 
